fit_lprk <- function(sim.list, bfs, scenario.str = scenario) {
  start.time <- Sys.time()
  ...
  if (bfs == "search") {
    ...
    search.res <- basis_search(sim.list, approx_type = "Lp")
    sqrt_bf_nodes <- search.res[which.max(search.res[,2]), 1]
  } else {
    sqrt_bf_nodes <- as.numeric(strsplit(bfs, "x", fixed = T)[[1]][1])
  }
  bfns <- make_basis(sqrt_bf_nodes, data = dat, radius.type = "limiting")
  # bfns <- switch(bfs,
  ...
  # start.pars <- list(b0 = 0, b1 = 0, u = rep(0, ncol(dat.list$Z_pres)), logdel = 0)
  ...
  if (!exists("res")) {
    ...
    if (!exists("res.estimates")) {
      ...
      # tmp.post_vars <- NA
      # tmp.prior_var <- as.numeric(res$par["logdel"])
      ...
      tmp.betas <- rbind(tmp.b0, tmp.b1)
      tmp.mus <- res.estimates[rownames(res.estimates) == "u", 1]
      # tmp.post_vars <- NA
  ...
  # save the result
  save(list = "sim.res", file = paste0("Results/", scenario.str, "/res_lprk_", bfs, "_sim_", job, ".RDATA"))
}