...
########### Elements To Change ##############
Int = 1.3471655 # Expected N = 100000
cov.par = 20 # Smooth
range.par = 10 # Wiggly
Slope = 1.25
cor.fn = "stable"
...
scenario <- "SW_N_100000_stable_large_quad"
...
# fit VARK (variational approx. with fixed rank kriging) with radius of gap x 1.5
if (!paste0("res_vark_35x35_diag_sim_", job, ".RDATA") %in% list.files(paste0("Results/", scenario))) {
  fit_vark_cold(sim.list, bfs = "35x35", radius_type = "diag")
}
# fit VARK (variational approx. with fixed rank kriging) with limiting radius
if (!paste0("res_vark_35x35_limiting_sim_", job, ".RDATA") %in% list.files(paste0("Results/", scenario))) {
  fit_vark_cold(sim.list, bfs = "35x35")
}

## VARK/LPRK with 42x42 basis functions:
# fit VARK (variational approx. with fixed rank kriging) with radius of gap x 1.5
if (!paste0("res_vark_42x42_diag_sim_", job, ".RDATA") %in% list.files(paste0("Results/", scenario))) {
  fit_vark_cold(sim.list, bfs = "42x42", radius_type = "diag")
}
# fit VARK (variational approx. with fixed rank kriging) with limiting radius
if (!paste0("res_vark_42x42_limiting_sim_", job, ".RDATA") %in% list.files(paste0("Results/", scenario))) {
  fit_vark_cold(sim.list, bfs = "42x42")
}

# fit LPRK (Laplace approx. with fixed rank kriging)
if (!paste0("res_lprk_42x42_sim_", job, ".RDATA") %in% list.files(paste0("Results/", scenario))) {
  fit_lprk(sim.list, bfs = "42x42")
}
