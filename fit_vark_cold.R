fit_vark_cold <- function(sim.list, bfs, scenario.str = scenario, radius_type = "limiting") {
  start.time <- Sys.time()
  ...
  if (!exists("res")) {
    sim.res <- data.frame(ftime = NA,
                 ...
                 atime = NA,
                 model = paste0("vark_", bfs, "_", radius_type)
                 )
  ...
  # save the result
  save(list = "sim.res", file = paste0("Results/", scenario.str, "/res_vark_", bfs, "_", radius_type, "_sim_", job, ".RDATA"))
}